"""
iPPG Advanced Full Pipeline (Combined Step 4 & 5)
===================================================
...
from ippg_common_adv import (
    ...
    load_video_mean_intensity, load_cppg,
...
def main():
    ...
    print("=" * 60)
    print("  iPPG ADVANCED FULL PIPELINE (STEP 4 + 5)")
    print("=" * 60)

    # 1. Load Data
    raw_intensity, t_ippg, fps = load_video_mean_intensity()
    ...
    I0 = np.mean(raw_intensity)
    od_signal = -np.log(raw_intensity / I0 + 1e-9)
    od_detrended = detrend(od_signal)
    ...
    for order in orders:
        for sg_w in sg_windows:
            try:
                ...
                ippg_f = bandpass_filter(sg_sig, fps, low=BP_LOW, high=BP_HIGH, order=order)

                passed, info, reasons = physiological_check(
                    ippg_f, fps, duration, ref_hr_bpm, fpeak_c
                )
                ...
                if passed:
                    evaluate_quality(candidate, t_ippg, cppg_ref, t_cppg, ref_hr_bpm, fpeak_c)
    ...
    if not valid:
        print("\n❌ All candidates failed physiological gating. Cannot proceed to FFT.")
        return
    ...
    best_json = os.path.join(OUT_DIR, f"best_order_adv_{WAVELENGTH}.json")
    with open(best_json, 'w') as f:
        json.dump({'best_bw': best_bw, 'best_sg': best_sg, 'best_score': float(best_score)}, f, indent=2)
        
    # Save signals for comparison
    np.save(os.path.join(OUT_DIR, f"ippg_signal_{WAVELENGTH}.npy"), best['signal'])
    np.save(os.path.join(OUT_DIR, f"ippg_time_{WAVELENGTH}.npy"), t_ippg)
    cppg_filtered = bandpass_filter(detrend(ir_raw_cppg), fs_cppg, order=2)
    np.save(os.path.join(OUT_DIR, f"cppg_signal_{WAVELENGTH}.npy"), cppg_filtered)
    np.save(os.path.join(OUT_DIR, f"cppg_time_{WAVELENGTH}.npy"), t_cppg)
    ...
    ax1.set_title(f"Step 4: Advanced Sweep Candidate Scores | {WAVELENGTH}", pad=15, fontweight='bold')
    ...
    plt.tight_layout()
    out1 = os.path.join(OUT_DIR, f"step4_advanced_sweep_{WAVELENGTH}.png")
    fig1.savefig(out1, dpi=300, bbox_inches='tight')
    plt.close(fig1)
    print(f"  ✅ Saved Heatmap: {out1}")

    # ==========================================
    # 7. FFT & Morphology Analysis (Step 5 part)
    # ==========================================
    ippg_final = best['signal']
    
    freq_i, mag_i = compute_fft(ippg_final, fps)
    hr_ippg, fpeak_i = find_hr_peak(mag_i, freq_i)
    snr_db = compute_snr(ippg_final, fps, f_low=BP_LOW, f_high=BP_HIGH)
    
    print(f"\n  [FFT Analysis on Best Signal]")
    print(f"  iPPG → f_peak = {fpeak_i:.3f} Hz → HR = {hr_ippg:.1f} BPM")
    print(f"  iPPG → SQI (SNR) = {snr_db:.2f} dB")
    ...
    hr_error = abs(hr_ippg - hr_cppg)
    print(f"  |Error| = {hr_error:.1f} BPM")

    # Extract Morphology
    peaks, troughs, log = detect_valid_peaks(ippg_final, fps, hr_cppg)
    beats = segment_beats(ippg_final, t_ippg, peaks, troughs, fps)
    ...
    ax_time.plot(t_ippg, z_normalize(ippg_final), color='#55a868', lw=1.5, label=f'NIR-iPPG {WAVELENGTH}')
    ...
    plt.tight_layout()
    out_img_time = os.path.join(OUT_DIR, f"step4_5_time_domain_5to10s_{WAVELENGTH}.png")
    ...
    ax_fft.plot(freq_i[fmask], mag_norm_i[fmask], color='#d65f5f', lw=2, label=f'NIR-iPPG {WAVELENGTH}')
    ax_fft.plot(freq_c[fmask], mag_norm_c[fmask], color='black', ls='--', lw=1.5, alpha=0.7, label='cPPG (Ref)')
    
    ax_fft.axvline(fpeak_i, color='#d65f5f', ls=':', lw=1.5, label=f'iPPG Peak: {fpeak_i:.2f}Hz ({hr_ippg:.1f} BPM)')
    ...
    plt.tight_layout()
    out_img_fft = os.path.join(OUT_DIR, f"step4_5_fft_{WAVELENGTH}.png")
