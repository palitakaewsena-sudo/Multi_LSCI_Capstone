"""
iPPG Pipeline — Shared Configuration & Utilities
===================================================
config file, helper functions, and data loading shared across all Steps
"""
...
OUT_DIR = os.path.join(r"../../results/ippg", DATASET_NAME)
...
def load_video_mean_intensity(video_path: str = VIDEO_PATH, dark_frame_path: str = DARK_FRAME_PATH):
    """
    Load video and calculate Mean ROI Intensity for each frame
    Returns: (raw_intensity, time_axis, fps)
    """
    ...
    while True:
        ...
        # Extract central 20% ROI (tightened)
        h, w = frame.shape[:2]
        r_start, r_end = int(h * 0.40), int(h * 0.60)
        c_start, c_end = int(w * 0.40), int(w * 0.60)
        frame_roi = frame[r_start:r_end, c_start:c_end]
        ...
        if dark_frame is not None:
            df_roi = dark_frame[r_start:r_end, c_start:c_end]
            fh, fw = pixel_data.shape
            df_roi = df_roi[:fh, :fw]
            pixel_data = np.maximum(pixel_data - df_roi, 0.0)
            
        mean_intensities.append(np.mean(pixel_data))
        frame_idx += 1
    cap.release()

    raw_intensity = np.array(mean_intensities)
    time_axis = np.arange(len(raw_intensity)) / fps + TIME_START
    print(f"  Extracted {len(raw_intensity)} frames (Central 50% ROI, starting {TIME_START}s) → {time_axis[-1]:.1f} s")

    return raw_intensity, time_axis, fps
